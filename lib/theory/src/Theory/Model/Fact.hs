...
data MonotonicBehaviour = Constant | Increasing | Decreasing | StrictlyIncreasing | StrictlyDecreasing | Unstable | Unspecified
  deriving (Eq, Ord, Data, Generic, NFData, Binary)
...
data Fact t = Fact
    ...
    , injectiveBehavior :: Maybe [[MonotonicBehaviour]]
    , factTerms         :: [t]
    }
    deriving( Show, Typeable, Data, Generic, NFData, Binary )

isInjective :: Fact t -> Bool
isInjective = isJust . injectiveBehavior
...
instance Eq t => Eq (Fact t) where
    (==) (Fact tag _ _ ts) (Fact tag' _ _ ts') = (tag == tag') && (ts == ts')

instance Ord t => Ord (Fact t) where
    compare (Fact tag _ _ ts) (Fact tag' _ _ ts') = compare tag tag' <> compare ts ts'

instance Functor Fact where
    fmap f (Fact tag an i ts) = Fact tag an i (fmap f ts)

instance Foldable Fact where
    foldMap f (Fact _ _ _ ts) = foldMap f ts

instance Traversable Fact where
    sequenceA (Fact tag an i ts) = Fact tag an i <$> sequenceA ts
    traverse f (Fact tag an i ts) = Fact tag an i <$> traverse f ts

instance Sized t => Sized (Fact t) where
  size (Fact _ _ _ args) = size args
...
kdFact = Fact KDFact S.empty Nothing . return
kuFact = Fact KUFact S.empty Nothing . return
termFact = Fact TermFact S.empty Nothing . return

-- | Make annotated KU/KD facts
kdFactAnn, kuFactAnn :: S.Set FactAnnotation -> t -> Fact t
kdFactAnn ann = Fact KDFact ann Nothing . return
kuFactAnn ann = Fact KUFact ann Nothing . return

-- | View a message-deduction fact.
kFactView :: LNFact -> Maybe (DirTag, LNTerm)
kFactView fa = case fa of
    Fact KUFact _ _ [m] -> Just (UpK, m)
    Fact KUFact _ _ _   -> errMalformed "kFactView" fa
    Fact KDFact _ _ [m] -> Just (DnK, m)
    Fact KDFact _ _ _   -> errMalformed "kFactView" fa
    _                   -> Nothing

-- | View a deduction logging fact.
dedFactView :: LNFact -> Maybe LNTerm
dedFactView fa = case fa of
    Fact DedFact _ _ [m] -> Just m
    Fact DedFact _ _ _   -> errMalformed "dedFactView" fa
    _                    -> Nothing
...
dedLogFact :: t -> Fact t
dedLogFact = Fact DedFact S.empty Nothing . return

-- | A protocol fact denotes a fact generated by a protocol rule.
protoFact :: Multiplicity -> String -> [t] -> Fact t
protoFact multi name ts = Fact (ProtoFact multi name (length ts)) S.empty Nothing ts
...
protoFactAnn multi name an ts = Fact (ProtoFact multi name (length ts)) an Nothing ts

-- | Add annotations to an existing fact
annotateFact :: S.Set FactAnnotation -> Fact t -> Fact t
annotateFact ann' (Fact tag ann i ts) = Fact tag (S.union ann' ann) i ts

-- | Apply macros in fact
applyMacroInFact :: [Macro] -> LNFact -> LNFact
applyMacroInFact mcs (Fact tag annot i terms) = let mTerms = map (applyMacros mcs) terms in
                                                Fact tag annot i mTerms
...
isProtoFact (Fact ProtoFact {} _ _ _) = True
isProtoFact _                         = False

-- | True iff the fact is an input fact.
isInFact :: Fact t -> Bool
isInFact (Fact InFact _ _ _) = True
isInFact _                   = False
...
protoOrInFactView fa = case fa of
    Fact ProtoFact {} _ _  m  -> Just m
    Fact InFact       _ _ [m] -> Just [m]
    Fact InFact       _ _  _  -> errMalformed "protoOrInFactView" fa
    _                         -> Nothing
...
protoOrOutFactView fa = case fa of
    Fact ProtoFact {} _ _  m  -> Just m
    Fact OutFact      _ _ [m] -> Just [m]
    Fact OutFact      _ _  _  -> errMalformed "protoOrOutFactView" fa
    _                         -> Nothing
...
outFactView fa = case fa of
    Fact OutFact _ _ [m] -> Just m
    Fact OutFact _ _  _  -> errMalformed "outFactView" fa
    _                    -> Nothing
...
normFact (Fact h an i ts) = reader $ \hnd -> Fact h an i (map (\term -> runReader (norm' term) hnd) ts)
...
getLeftFact (Fact tag an i ts) =
   Fact tag an i (map getLeftTerm ts)

-- | Get "left" variant of a diff fact
getRightFact :: LNFact -> LNFact
getRightFact (Fact tag an i ts) =
   Fact tag an i (map getRightTerm ts)

-- | Get all variables inside a fact
getFactVariables :: LNFact -> [LVar]
getFactVariables (Fact _ _ _ ts) =
...
isTrivialFact (Fact _ _ _ ts) = case ts of
      []   -> Just []
...
prettyFact ppTerm (Fact tag an _ ts)
