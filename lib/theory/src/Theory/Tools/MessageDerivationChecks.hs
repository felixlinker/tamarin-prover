...
generateSeparatedLemmas idx vars = map (\v -> Lemma (show v) "message_deriv" ExistsTrace (existsTimeFormula $ existFormula $ landFormula $ generateAction vars idx : [(lntermToKUFact (lvarToLnterm v))]) [] (unproven ())) vars
