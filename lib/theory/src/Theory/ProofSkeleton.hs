...
unprovenLemma :: String -> [LemmaAttribute] -> TraceQuantifier -> LNFormula
              -> Lemma ProofSkeleton
unprovenLemma name atts qua fm = Lemma name "Unpr_inSkeleton" qua fm atts (unproven ())

skeletonLemma :: String -> String-> [LemmaAttribute] -> TraceQuantifier -> f -> p -> ProtoLemma f p
skeletonLemma name ptxt atts qua fm = Lemma name ptxt qua fm atts
